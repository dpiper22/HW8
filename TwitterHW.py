...
def get_tweets():
    if 'umsi' in CACHE_DICTION:
        ...
        #also saves to dictionary to cache
        CACHE_DICTION['umsi'] = twitter_results #add it to the dictionary and new key value pair
        # then writes whole cache dictionary with new info added, to file
        f = open(CACHE_FNAME, 'w') #open cache file for writing
        f.write(json.dumps(CACHE_DICTION)) #makes whole dictionary holding data
        f.close()
    return twitter_results # returns the list
